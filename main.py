"""GTP coding challenge server — one app for all stages.

Exposes:
  /mcp       MCP streamable-HTTP endpoint (Stage 1 "Nursery" tools)
  /solve     Adaptive API gateway (V2 -> V1 adaptation + SLO metrics)
...
async def solve(request: Request) -> JSONResponse:
    ...
    adapt_output = build_adapt_output(data.get("adaptInput") or {})
    slo_output = build_slo_output(data.get("heartbeats") or [], data.get("sloQuery") or {})

    return JSONResponse({"adaptOutput": adapt_output, "sloOutput": slo_output})
